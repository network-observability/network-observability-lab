...
class LogConfig(BaseModel):
    ...
    LOGGER_NAME: str = "machine-learning"
...
class Settings(BaseSettings):  # pylint: disable=too-few-public-methods
    ...
    debug: bool = False
    # prefect_api_url: str = "http://air:4200/api"
    # prefect_deployment_name: str = "alertmanager-webhook-receiver"
    host: str = "0.0.0.0"
    port: int = 9997
    ...
    class Config:  # pylint: disable=too-few-public-methods
        ...
        # env_prefix = "PREFECT_ALERTMANAGER_WEBHOOK_"
...
def load(config_file_name="pyproject.toml", config_data=None):
    ...
    # Or load from configuration file
        ...
        if config_file.exists():
            ...
            if "tool" in config_tmp and "loki-sender" in config_tmp.get("tool", {}):
                SETTINGS = Settings(**config_tmp["tool"]["loki-sender"])
...
def load_or_exit(config_file_name="pyproject.toml", config_data=None):
    ...
    When loaded from file, the settings for this app are expected to be in [tool.loki-sender] in TOML
    if nothing is found in the config file or if the config file do not exist, the default values will be used.
